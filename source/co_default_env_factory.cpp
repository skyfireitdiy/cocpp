...
co_env* co_default_env_factory::create_env(size_t stack_size)
{
    ...
    auto ret      = new co_default_env(idle_ctx, stack_factory__->create_stack(stack_size), true);
...
{
    auto idle_ctx = create_idle_ctx__();
    auto ret      = new co_default_env(idle_ctx, stack_factory__->create_stack(stack_size), false);
    idle_ctx->set_state(co_state::running);
    CO_DEBUG("create env: %p", ret);
    return ret;
}
...
{
    return ctx_factory__->create_ctx(co_ctx_config { nullptr, 0, nullptr, "idle" });
}