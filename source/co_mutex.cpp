...
void co_mutex::lock()
{
    ...
    {
        owner__ = ctx;
        CO_DEBUG("set owner: %p", owner__.load());
        spinlock__.unlock();
        return;
    }

    CO_DEBUG("add to wait list: %p", ctx);
...
void co_mutex::unlock()
{
    ...
    {
        CO_ERROR("ctx is not owner: %p owner is %p", ctx, owner__.load());
    ...
    owner__ = waked_ctx;
    CO_DEBUG("set owner: %p from %p", waked_ctx, ctx);
    spinlock__.unlock();
