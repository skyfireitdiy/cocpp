...
void ctx_enter_wait_state__(co_ctx* ctx, int rc_type, void* rc, std::list<ContextType>& wait_list, const ContextType& data)
{
...
template <typename ContextType>
void wake_front__(std::list<ContextType>& wait_list, std::function<void(ContextType&)> wake_method)
{
    if (wait_list.empty())
    {
        return;
